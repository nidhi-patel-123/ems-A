...
const Header = () => {
  ...
  useEffect(() => {
    ...
    socketRef.current = io("https://ems-b-a2p8.onrender.com", {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch("https://ems-b-a2p8.onrender.com/admin/notifications", {
        headers: {
  ...
  const markAsRead = async (id) => {
    try {
      await fetch(`https://ems-b-a2p8.onrender.com/notifications/${id}/read`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        },
  ...
  const deleteNotification = async (id) => {
    if (window.confirm("Are you sure you want to delete this notification?")) {
      try {
        const response = await fetch(`https://ems-b-a2p8.onrender.com/notifications/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            'x-user-id': sessionStorage.getItem("adminId") || '',
